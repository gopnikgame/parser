...
# Улучшенная функция для ввода токена
read_token() {
    local token
    
    log "BLUE" "🎯 Ожидание ввода токена..."
    
    # Простой и надёжный способ чтения с /dev/tty
    exec < /dev/tty
...
check_env_token() {
    log "BLUE" "🔍 Проверка переменной окружения GITHUB_TOKEN..."
    
    # Отладочная информация
    log "BLUE" "📊 Все переменные GITHUB_*:"
    env | grep GITHUB || log "YELLOW" "⚠️ Переменные GITHUB_* не найдены"
    
    if [ -n "${GITHUB_TOKEN:-}" ]; then
        local token_length=${#GITHUB_TOKEN}
        log "GREEN" "✅ Найден токен в переменной окружения (длина: $token_length символов)"
        echo "$GITHUB_TOKEN"
        return 0
    else
        log "YELLOW" "⚠️ Переменная GITHUB_TOKEN пуста или не установлена"
        return 1
    fi
}
...
# Основная функция для получения токена
setup_github_token() {
    local token
    local attempts=0
    ...
    log "BLUE" "🔍 Поиск токена в переменных окружения..."
    if token=$(check_env_token 2>/dev/null); then
        log "BLUE" "🔍 Проверяем токен из переменной окружения..."
        if validate_github_token "$token" && check_repo_access "$token"; then
            log "GREEN" "✅ Токен из переменной окружения принят"
            echo "$token"
            return 0
        else
            log "YELLOW" "⚠️ Токен из переменной окружения невалиден"
        fi
    fi
    
    # Интерактивный ввод
    log "BLUE" "🎯 Переход к интерактивному вводу токена..."
    echo ""
    ...
    while [ $attempts -lt $max_attempts ]; do
        ...
        if token=$(read_token 2>/dev/null); then
            ...
            if [ ${#token} -ge 16 ]; then
                ...
                log "BLUE" "🔍 Проверяем введенный токен..."
            fi
            ...
            if validate_github_token "$token" && check_repo_access "$token"; then
                ...
                log "RED" "❌ Токен невалиден или нет доступа к репозиторию"
            fi
        else
            log "YELLOW" "⚠️ Не удалось прочитать токен"
        fi
    ...
    echo ""
    log "YELLOW" "💡 Альтернативные решения:"
    echo "   1. Установите переменную окружения:"
    echo "      export GITHUB_TOKEN='your_token_here'"
    echo "      ./launcher_parser.sh  # БЕЗ sudo!"
    echo ""
    echo "   2. Или создайте .env файл вручную:"
    echo "      mkdir -p $INSTALL_DIR"
    echo "      echo 'GITHUB_TOKEN=your_token_here' > $INSTALL_DIR/.env"
    echo "      chmod 600 $INSTALL_DIR/.env"
    echo "      ./launcher_parser.sh"
    echo ""
...
log "BLUE" "👤 Пользователь: $(whoami)"
log "BLUE" "📍 Рабочая директория: $(pwd)"
...
if [ -d "$INSTALL_DIR" ]; then
    ...
    if ! github_token=$(setup_github_token); then
        log "RED" "❌ Не удалось настроить GitHub токен"
        log "BLUE" "💡 Попробуйте один из альтернативных способов, указанных выше"
        exit 1
    fi
