...
# Упрощённая функция для ввода токена под root
read_token() {
    local token
    
    # Проверяем доступность /dev/tty
    if [ -c /dev/tty ]; then
        log "BLUE" "🎯 Используем /dev/tty для ввода"
        echo -n "🔑 Введите ваш GitHub Personal Access Token: "
        
        # Отключаем эхо
        stty -echo 2>/dev/null || true
        
        # Читаем прямо из /dev/tty
        read token < /dev/tty
        ...
        # Обычный способ
        log "BLUE" "🎯 Используем стандартный ввод"
        echo -n "🔑 Введите ваш GitHub Personal Access Token: " >&2
        ...
        echo >&2  # Переход на новую строку
    fi
...
check_env_token() {
    if [ -n "${GITHUB_TOKEN:-}" ]; then
        log "GREEN" "✅ Найден токен в переменной окружения GITHUB_TOKEN"
        echo "$GITHUB_TOKEN"
        return 0
    fi
    return 1
...
setup_github_token() {
    ...
    # Интерактивный ввод
    log "BLUE" "🎯 Интерактивный ввод токена..."
    echo ""
